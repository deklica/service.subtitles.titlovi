...
#    releaseyear = re.findall('\(\d{4}\)', _string)
#    year = releaseyear[0].replace('(', '').replace(')', '')
    ...
    if season is not None and episode is not None:
        ...
        x,y,z = _string.lower().partition('s{0}e{1}'.format(results[0][0], results[0][1]))
        _string = x+y
        _string = _string.replace('s{0}e{1}'.format(results[0][0], results[0][1]), '')
...
class ActionHandler(object):
    ...
    def get_prepared_language_param(self):
        ...
        for lang in self.params['languages'][0].split(','):
            ...
            if lang == 'Serbian':
                if LANGUAGE_MAPPING['Serbian'] not in lang_list:
                    lang_list.append(LANGUAGE_MAPPING['Serbian'])
                if LANGUAGE_MAPPING['Cyrillic'] not in lang_list:
                    lang_list.append(LANGUAGE_MAPPING['Cyrillic'])
            else:
    ...
    def handle_search_action(self):
        ...
        if self.action == 'manualsearch':
            ...
            clean_tv_show_title, year = xbmc.getCleanMovieTitle(tv_show_title)
            if tv_show_title:
                search_params['query'] = clean_tv_show_title
                if season is not None:
                    search_params['season'] = season
                if episode is not None:
                    search_params['episode'] = None
        ...
        for result_item in result_list:
            ...
            if season > 0 and episode == 0:
                title = '{0} S{1}'.format(title, season)
                
            if season == 0 and episode == 0:
                title = '{0} SVE EPIZODE'.format(title)
    ...
    def handle_download_action(self):
        ...
        """
        subtitle_folder_name = 'titlovi_com_subtitle_{0}_{1}'.format(self.params['media_id'][0], self.params['type'][0])
        subtitle_folder_path = os.path.join(temp_dir, subtitle_folder_name)
        subtitle_files = sorted([(root, subtitle) for root, dirs, files in os.walk(subtitle_folder_path) for subtitle in files])
        ...
        if not subtitle_files:
            ...
            zip_contents = sorted(zip_file.namelist())
            if not zip_contents:
                show_notification(get_string(32010))
                return

            logger('zip_contents: {0}'.format(zip_contents))
            zip_file.extractall(str(subtitle_folder_path))
